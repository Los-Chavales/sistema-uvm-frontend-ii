...
class Events_Model {
  ...
  search_events_month(dateStart, dateFinish) {
    return new Promise((resolve, reject) => {
      connection.query('SELECT * FROM `fechas_especiales` WHERE `fecha_especial` < ? AND `fecha_especial` >= ?;', [dateFinish, dateStart], function (error, results, fields) {
        if (error) {
          reject(new Response(500, error, error));
        } else {
          if (results.length == 0) {
